function cons = constraints_RPO(x, M, S, C, N, nPar_rf, lmt, par_mus,...
                                 t_em, t_rf, hs, phase, tPhase)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% The function for calculating the constraints that can handle the muscle
% electromechanical and reflex control constant delays.
%
% By: Huawei Wang
% Date: March 13, 2022
...
% OUTPUT:
%   cons, vector: the constraint vector of the systme dynamics
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    rCons = M*C*sum(N - 1 - ceil((t_em + t_rf)./hs)) ...  % number of full elements constraints
            + M*(C-1)*sum(ceil((t_em + t_rf)./hs) - ceil(t_em./hs)) ... % number of constrinats that does not have reflex loop
            + M*(C-2)*sum(ceil(t_em./hs));  % number of constraints that does not have reflex and activation dynamic loops
    ...
    for t = 1:length(N)      % Run through all data trials
        ...
        cons_st = M*C*sum(N(1:t-1) - ceil((t_em(1:t-1) + t_rf(1:t-1))./hs(1:t-1)) - 1) ...
            + M*(C-1)*sum(ceil((t_em(1:t-1) + t_rf(1:t-1))./hs(1:t-1))...
            - ceil(t_em(1:t-1)./hs(1:t-1))) ...
        ...
        for n = 1:N(t)-1    % Run through all data nodes (using middle point method)
            ...
            if n <= ceil(n_em)
               ...
               cons(cons_st + (n-1)*M*(C-2)+1:cons_st + n*M*(C-2)) =...
                   directCollocationDyn_RPO1(x_tn, dx_tn, lmt_tn, par_mus, M);
            ...
            elseif n <= ceil(n_em + n_rf)
                ...
                % constraints
                cons(cons_st + cons_st_em + (n-ceil(n_em)-1)*M*(C-1)+1 ...
                    :cons_st + cons_st_em + (n-ceil(n_em))*M*(C-1)) =...
                   directCollocationDyn_RPO2(x_tn, dx_tn, x_em_tn, lmt_tn,...
                                             par_mus, M);
            ...
            % reflex control loops, include the reflex control constraints.
                ...
                % directCollocationDyn_RPO3 is the function that contains 
                % all dynamic constraints 
                cons(cons_st + cons_st_em + cons_st_rf + (n- ceil(n_em + n_rf) -1)*M*C+1 ...
                    :cons_st + cons_st_em + cons_st_rf + (n- ceil(n_em + n_rf))*M*C)...
                    = directCollocationDyn_RPO3(x_tn, dx_tn, x_em_tn, x_rf_tn,...
                            lmt_tn, lmt_rf_tn, par_rf_fse_tn, par_rf_lce_tn, ...
                            par_rf_res, par_mus, M);
