...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    [fse_rf, dfse_rf_dlce_rf]...  % muscle force and its derivative
            = tendenForce_Groote_diff_RPO(lmt_rf, lce_rf, lce_opt, lt_slack, theta0);
    
    % normalize the lce length with the LceOpt
    rlce_rf = lce_rf./lce_opt;  % normalize lce lengths [0.5, 1.5]
    drlce_rf_dlce_rf = 1./lce_opt; 
    
    % generate muscle stimulation based on the reflex control
    % s_rf = fullReflexController(lce_rf, fse_rf, phase_rf, par_rf);  
    [ds_rf_drlce_rf, ds_rf_dfse_rf, dfs_rf_dpar_rf_fse, dfs_rf_dpar_rf_lce, ...
        dfs_rf_dpar_rf_res] = fullReflexController_diff(rlce_rf, fse_rf, par_rf_fse,...
        par_rf_lce, par_rf_res, M);
    ...
    % controller
    % fs = s_rf - s_em;
    dfs_dx_rf = zeros(M, S*M);  % calculate the jacobian 
    dfs_dx_em = zeros(M, S*M);
    
    dfs_dx_rf(:, 2*M+1 : 3*M) = drlce_rf_dlce_rf.*ds_rf_drlce_rf + ...
                                dfse_rf_dlce_rf.*ds_rf_dfse_rf;

    dfs_dx_em (:, 4*M+1 : 5*M) = -eye(M);
