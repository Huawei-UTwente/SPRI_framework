...
a_rf = 0.8*rand(1, M) + 0.01;
da_rf = 1 - 2*rand(1, M);

lce_rf = 0.03 + 0.02*rand(1, M);
dlce_rf = -0.1 + 0.2*rand(1, M);

s_rf = 0.8*rand(1, M) + 0.01;
...
par_rf_fse = 5 - 10*rand(1, M*M);
par_rf_lce = 5 - 10*rand(1, M*M);
par_rf_res = [zeros(1, M), 0.5 + rand(1, M), 0.05*rand(1, M)];

% the derivative functions
 [df_dx, df_ddx, df_dx_em, dfs_dx_rf, dfs_rf_dpar_rf_fse, ...
    dfs_rf_dpar_rf_lce, dfs_rf_dpar_rf_res] = directCollocationDyn_diff_RPO3(x, dx, ...
    x_em, x_rf, lmt, lmt_rf, par_rf_fse, par_rf_lce, par_rf_res, par_mus, M, S);

df_dx_rf = zeros(M*C, M*S);
df_dx_rf(end-M+1:end, :) = dfs_dx_rf;
...
for ia = 1:length(x)
   ...
   x(ia) = x(ia) + delta_x;
   f_up = directCollocationDyn_RPO3(x, dx, x_em, x_rf, lmt, lmt_rf,...
             par_rf_fse, par_rf_lce, par_rf_res, par_mus, M);
   
   % get values with upper change
   x(ia) = x(ia) - 2*delta_x;
   f_dw = directCollocationDyn_RPO3(x, dx, x_em, x_rf, lmt, lmt_rf,...
             par_rf_fse, par_rf_lce, par_rf_res, par_mus, M);
...
delta = 1e-4;
% differentiation of input parameters
for ia = 1:length(par_rf_fse)
    
   % get values with upper change
   delta_x = par_rf_fse(ia)*delta;

   par_rf_fse(ia) = par_rf_fse(ia) + delta_x;
   f_up = directCollocationDyn_RPO3(x, dx, x_em, x_rf, lmt, lmt_rf,...
             par_rf_fse, par_rf_lce, par_rf_res, par_mus, M);

   % get values with upper change
   par_rf_fse(ia) = par_rf_fse(ia) - 2*delta_x;
   f_dw = directCollocationDyn_RPO3(x, dx, x_em, x_rf, lmt, lmt_rf,...
             par_rf_fse, par_rf_lce, par_rf_res, par_mus, M);
   ...
   % calculate the finite differentiation
   dfs_dpar_rf_fse_fd(:, ia) = (f_up(end-M+1:end) - f_dw(end-M+1:end))/(2*delta_x);
...
delta = 1e-4;
% differentiation of input parameters
for ib = 1:length(par_rf_lce)

   % get values with upper change
   delta_x = par_rf_lce(ib)*delta;

   par_rf_lce(ib) = par_rf_lce(ib) + delta_x;
   f_up = directCollocationDyn_RPO3(x, dx, x_em, x_rf, lmt, lmt_rf,...
             par_rf_fse, par_rf_lce, par_rf_res, par_mus, M);

   % get values with upper change
   par_rf_lce(ib) = par_rf_lce(ib) - 2*delta_x;
   f_dw = directCollocationDyn_RPO3(x, dx, x_em, x_rf, lmt, lmt_rf,...
             par_rf_fse, par_rf_lce, par_rf_res, par_mus, M);
   ...
   % calculate the finite differentiation
   dfs_dpar_rf_lce_fd(:, ib) = (f_up(end-M+1:end) - f_dw(end-M+1:end))/(2*delta_x);
