...
save_path = 'ReflexControllerOptResults';
...
for t = 1:T  % load trial data and get muscle parameters from the averaged gait data
    ...
    t_em(t) = hs(t)*6; % electromechanical delay
...
grad_equ = gradient_RPO(x, M, S, N, nPar_rf, lmt, par_mus,...
                            tPhase, W1, W2, W3, w11, w12, w13, w14, torque, mus_act, ma);
...
for ia = 1:length(x)
   ...
   x(ia) = x(ia) + delta_x;
   obj_up = objective_RPO(x, M, S, N, nPar_rf, lmt, par_mus,...
     tPhase, W1, W2, W3, w11, w12, w13, w14, torque, mus_act, ma);
   
   % get values with upper change
   x(ia) = x(ia) - 2*delta_x;
   obj_dw = objective_RPO(x, M, S, N, nPar_rf, lmt, par_mus,...
     tPhase, W1, W2, W3, w11, w12, w13, w14, torque, mus_act, ma);
