...
[df_dx, df_ddx] = directCollocationDyn_diff_RPO1(x, dx, lmt, par_mus, M, S);
...
for ia = 1:length(x)
   ...
   x(ia) = x(ia) + delta_x;
   f_up = directCollocationDyn_RPO1(x, dx, lmt, par_mus, M);
   
   % get values with upper change
   x(ia) = x(ia) - 2*delta_x;
   f_dw = directCollocationDyn_RPO1(x, dx, lmt, par_mus, M);
