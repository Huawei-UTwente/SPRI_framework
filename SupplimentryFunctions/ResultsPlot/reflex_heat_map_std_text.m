...
function hf = reflex_heat_map_std_text(ref_gains_mean, ref_gain_std, M, cphase, phase_occupied)
    ...
    prf_fse_matrix_std = reshape(ref_gain_std(1:M*M*cphase), M, M, cphase);  % fse reflex gains
    prf_lce_matrix_std = reshape(ref_gain_std(M*M*cphase+1:M*M*cphase*2), M, M, cphase);  % lce reflex gains
    ...
    for m = 1:M
        ...
        for mm = 1:M
            for p = 1:cphase
                ...
                hold on
                text((hphase(p) + hphase(p+1))/2, ...
                    m*vblock + (m-1)*ratio_fill_blank*vblock + (mm-0.5)*vmblock,...
                    num2str(round(prf_fse_matrix_std(m, mm, p), 3)),...
                    'FontSize', 7)
            end
        end
    end
    ...
    for m = 1:M
        ...
        for mm = 1:M
            for p = 1:cphase
                ...
                text((hphase(p) + hphase(p+1))/2, ...
                    m*vblock + (m-1)*ratio_fill_blank*vblock + (mm-0.5)*vmblock,...
                    num2str(round(prf_lce_matrix_std(m, mm, p), 3)),...
                    'FontSize', 7)
            end
        end
    end
    ...
    for m = 1:M
        ...
        rectangle('Position',[0.02,...
                    ...
                    0.04, vmblock*M*ref_gains_mean(M*M*5*2+m)],...
    ...
    for m = 1:M
        ...
        rectangle('Position',[0.02,...
                    m*vblock + (m-1)*ratio_fill_blank*vblock,...
                    0.04, vmblock*M*ref_gains_mean(M*M*5*2+M+m)],...
